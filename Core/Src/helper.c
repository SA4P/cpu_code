...
// Sets up GPIO pins for the specified UART. Special for LPUART1 is that we use it in two modes: (1) connected to on-board serial-USB converter, (2) connected to GPIO pins. If uart == lpuart and to_usb == TRUE, we configure LPUART to go to the USB port
void gpio_init_uart(USART_TypeDef * uart, bool_t to_usb){
	...
	if (uart == LPUART1){										// Case: Configure GPIO pins for LPUART1 ==> Pins PG7 and PG8 (i.e. 92, 93) in our case
		...
		if(to_usb){
		// CASE: Connect LPUART1 via internal serial-USB converter to USB port
		/*
		...
		gpio->AFR[0] |= GPIO_AF8_LPUART1 << 28;						// GPIO_AF8_LPUART1 has value 0b1000, which in turn is shifted by 28 bits such that the bit at position 3 (starting at 0) arrives at bit 31, the MSB of AFRL which corresponds to the 3rd bit (starting at 0) of PG7. This is the pin we need to set to 1
		...
		gpio->MODER ^= (0x1 << 14 | 0x01 << 16);	// MODER is by default set to 0b1111...1 (i.e. all 1 bits). We want the 7th and 8th pin (with bit fields 15:14, 17:16 resp.) to have value 0b10 (Alternative function mode)
			...
			// CASE: Connect LPUART1 to GPIO pins PC0 and PC1
			gpio = GPIOC;
			gpio->AFR[0] |= GPIO_AF8_LPUART1 | (GPIO_AF8_LPUART1 << 4);						// GPIO_AF8_LPUART1 has value 0b1000, which we for PC1 also have to shift for 4 bits (4 bits per pin, the lowest 4 bits are for PC0)
			gpio->MODER ^= 0x1  | (0x01 << 2);												// MODER is by default set to 0b1111...1 (i.e. all 1 bits). We want the 0th and 1st pin (with bit fields 1:0, 3:2 resp.) to have value 0b10 (Alternative function mode)
...
bool_t s_msg_build_header(byte_t msg_type, send_msg_t* o_msg){
	o_msg->msg_buf[0] = msg_type;

	if(msg_type == SERVER_PAYLOAD_SIGNUP_REQ){
			// TODO: Check if this line is correct
			uint8_t dev_cap_len = strlen(dev_state.dev_cap);
			o_msg->msg_buf[1] = LEN_SERVER_PAYLOAD_SIGNUP_REQ + dev_cap_len;			// NOTE: We do NOT include the 0-byte string-terminator of the device capabilities URI!
			o_msg->msg_buf[2] = 0;

			o_msg->len = HEADER_LEN + LEN_SERVER_PAYLOAD_SIGNUP_REQ + dev_cap_len;
			return TRUE;
		}
	else if(msg_type == SERVER_PAYLOAD_AUTH_REQ){
		// TODO: Check if this line is correct
		o_msg->msg_buf[1] = LEN_SERVER_PAYLOAD_AUTH_REQ;																							// NOTE: uint16_t has 2 bytes, but our messages (so far) have lengths that fit into one byte ==> Just write to that one byte
		o_msg->msg_buf[2] = 0;

		o_msg->len = HEADER_LEN + LEN_SERVER_PAYLOAD_AUTH_REQ;
...
// NOTE: i_buf only used for key material of the signup handshake!
bool_t s_msg_build(byte_t msg_type, req_t* req_state, msg_t* i_msg, send_msg_t* o_msg){
	...
	if(!s_msg_build_header(msg_type, o_msg)){
		while(TRUE){
			__NOP();
		}
	}
	...
	case SERVER_PAYLOAD_SIGNUP_REQ:
		memcpy(o_payload, i_msg->p_payload, LEN_GATEWAY_PAYLOAD_SIGNUP);							// i_buf already has correct format: dev_type(2 bytes) | keying_material(64 bytes)
		memcpy(o_payload + LEN_GATEWAY_PAYLOAD_SIGNUP, dev_state.dev_cap, strlen(dev_state.dev_cap));

		// Signup request has format: | header(3 bytes) | dev_type(2 bytes) | keying_material(64 bytes) | device_capabilities(variable) |
		return TRUE;

	case SERVER_PAYLOAD_AUTH_REQ:
		memcpy(o_payload, &dev_state.dev_id, DEVICE_ID_LEN);
		// memcpy(o_payload + DEVICE_ID_LEN, &req_state->access_type, ACCESS_TYPE_LEN);
...
// - - - - - - - - - - - - - - -
// Gateway Message building
// - - - - - - - - - - - - - - -

bool_t gw_msg_build_header(byte_t msg_type, send_msg_t* o_msg){
	o_msg->msg_buf[0] = msg_type;

	if(msg_type == GATEWAY_PAYLOAD_REQUEST){
			// TODO: Check if this line is correct
			o_msg->msg_buf[1] = LEN_GATEWAY_PAYLOAD_REQUEST;			// NOTE: We do NOT include the 0-byte string-terminator of the device capabilities URI!
			o_msg->msg_buf[2] = 0;

			o_msg->len = HEADER_LEN + LEN_GATEWAY_PAYLOAD_REQUEST;
			return TRUE;
		}
	else if(msg_type == GATEWAY_PAYLOAD_RESPONSE){
		// TODO: Check if this line is correct
		o_msg->msg_buf[1] = LEN_GATEWAY_PAYLOAD_RESPONSE;																							// NOTE: uint16_t has 2 bytes, but our messages (so far) have lengths that fit into one byte ==> Just write to that one byte
		o_msg->msg_buf[2] = 0;

		o_msg->len = HEADER_LEN + LEN_GATEWAY_PAYLOAD_RESPONSE;

		return TRUE;
	}
	else if(msg_type == GATEWAY_PAYLOAD_CONTROL){
		// Not implemented
		while(TRUE){
			__NOP();
		}
	...
	else if(msg_type == GATEWAY_PAYLOAD_SIGNUP_RESP){

		o_msg->msg_buf[1] = LEN_GATEWAY_PAYLOAD_SIGNUP_RESP;
		o_msg->msg_buf[2] = 0;

		o_msg->len = HEADER_LEN + LEN_GATEWAY_PAYLOAD_SIGNUP_RESP;
...
// Builds a challenge message given the challenge stored in the request state
// NOTE: For signup, we just copy i_buf over
bool_t gw_msg_build(byte_t msg_type, req_t* req_state, byte_t* i_buf, send_msg_t* o_msg){
	...
	// Check for error in header creation ==> Sign that passed msg_type is incorrect
	if(!gw_msg_build_header(msg_type, o_msg)){
		while(TRUE){
			__NOP();
		}
	}
	...
	case GATEWAY_PAYLOAD_SIGNUP_RESP:
		memcpy(o_payload, i_buf, LEN_GATEWAY_PAYLOAD_SIGNUP_RESP);
		return TRUE;
	default:
		return FALSE;
	}
 ...
 *
 * TODO: Make two versions, one for GW communication and one for server communication
 */
